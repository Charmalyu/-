...
public class SelfController {
    ...
    public void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String password = request.getParameter("password");   // 获取用户提交的账号和密码

        Staff staff = selfService.login(account, password); // 传入的账号和密码传给staff对象
        if (staff==null) {
            response.sendRedirect("toLogin.do");   // 登陆失败后跳转
        }else {
            HttpSession session = request.getSession();
            session.setAttribute("USER", staff);
            response.sendRedirect("main.do");  // 登陆成功把staff对象承载进session
        }
    }
